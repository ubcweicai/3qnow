...
<body>
			...
			<form:form modelAttribute="mediaQuery" method="post"
				...
				<div class="row myrow">
					<div class="col-md-12">
						...
						<a href="create.html" class="btn btn-success btn-sm openEditPage">上传图片</a>
						<button type="button" class="btn btn-warning btn-sm" onclick="deleteImages()">删除选定图片</button>
					</div>
				</div>
						...
						<c:forEach items="${mediamnglist}" var="var" varStatus="rowCounter">
							<li class="col-md-2 picitem">
								...
								<div class="row">
									<div class="checkbox">
										<label> 
										    <input type="checkbox" value="${var.media_id}" class="mediacheckbox"> <b>${var.title}</b> 
										</label>
										&nbsp;&nbsp;&nbsp;&nbsp;<a class="openEditPage" href="update.html?media_id=${var.media_id}"><span class="glyphicon glyphicon-pencil"></span></a>    
									</div>
								</div>
								<div class="row">
	...
	function deleteImg(id){
     ...
     $(document).ready(function(){ 
	    ...
	    $(".openEditPage").colorbox({iframe:true, width:"70%", height:"50%", onClosed:function(){ 
	    	location.reload();
	    }});
	    
	    $(".openViewPage").colorbox({iframe:true, width:"70%", height:"50%", onClosed:function(){ } });
