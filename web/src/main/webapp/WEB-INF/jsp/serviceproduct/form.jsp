...
<!-- 			</div> -->
				...
				<form:form id="myform" modelAttribute="serviceProduct" method="post"
					...
					<form:hidden path="recommend_level_id" />
					...
					<div class="row ">
						<div class="col-md-12">
							<div class="form-group">
								...
								<c:if test="${serviceProduct.operate=='add'}">
									<div class="col-md-8 item-content" >
										<form:select path="member_id" class="form-control dateSelector" data-validation="required">
						                   <form:option value="">--选择服务商ID--</form:option>
									...
									<div id="categoryListDiv">
										<c:forEach items="${memberCategoryMap}" var="var" varStatus="rowCounter">
											<form:label path="category_id" class="radio-inline">
										  		<form:radiobutton path="category_id" name="inlineRadioOptions" value="${var.key}" data-validation="required"/> ${var.value}
											</form:label>										
										</c:forEach>
									</div>
							...
							<div class="form-group">
								...
								<div class="col-md-8 item-content">
									...
									<c:forEach items="${applicationScope.cityMap}" var="var" varStatus="rowCounter">
										<div class="col-md-3 item-content myrow">
											<form:label path="serviceAreaList" class="checkbox-inline">
												<form:checkbox path="serviceAreaList" value="${var.key}"/>${var.value}
											</form:label>
										</div>
									</c:forEach>
									<div id="alertMessageDiv3"></div>
								</div>
							</div>
						</div>
					</div>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$('input[name=recommend_level_id]').attr("disabled",true);
