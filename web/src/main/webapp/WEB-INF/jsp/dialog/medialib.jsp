						...
						<c:forEach items="${mediamnglist}" var="var" varStatus="rowCounter">
							<li class="col-md-2 picitem">
								...
								<div class="row">
									<div class="checkbox">
										<label> 
										    <input type="checkbox" value="${var.media_id}" class="mediacheckbox" src="${var.bigpic}" title="${var.title}"> <b>${var.title}</b> 
										</label>
  ...
  function insertMultipleImgtoList(){
	 ...
	 for(i=0;i<imglist.size();i++){
		...
		if($(img).prop("checked")){
		   ...
		   var appendhtml = "<img src=\""+imgpath+"\" class=\"img-responsive\" title=\""+imgtitle+"\">";
		   parent.$("#insertHtml").append(appendhtml);
		}
	 }
